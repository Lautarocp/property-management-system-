    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {errors.zipCode && <p className="text-red-500 text-xs mt-1">Required</p>}
        </div>
        <div className="col-span-2">
...
export function ComplexesPage() {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Delete this complex?')) deleteComplex.mutate(id)
  }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Complexes</h2>
          <p className="text-gray-500 text-sm mt-1">{complexes?.length ?? 0} properties</p>
        </div>
        <button
      ...
      {(showCreate || editing) && (
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          <h3 className="text-lg font-semibold mb-4">{editing ? 'Edit Complex' : 'New Complex'}</h3>
          <ComplexForm
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">No complexes yet. Add your first one!</div>
      ) : (
            ...
            <div key={complex.id} className="bg-white rounded-xl shadow-sm p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                  {(complex as any)._count?.apartments ?? 0} apts
                </span>
              </div>
              {complex.description && (
